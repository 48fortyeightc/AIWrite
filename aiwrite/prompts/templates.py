...
3. **禁止使用以下表达**：
   ...
   - 禁止使用"如图X所示"、"如表Y所示"（除非写作要点中明确要求插入图表）
4. **语言要求**：
   - 使用学术规范的中文表达
   - 句式多样化，避免重复的句型结构
   - 每个段落开头用不同的方式引入
...
5. **内容要求**：
   ...
   - 保持逻辑连贯但避免机械衔接
   - 使用有效的 LaTeX 语法
6. **图表占位符**（仅当小节的 notes 中要求插入图表时使用）：
   - 在需要插入图片的地方使用：{{{{FIGURE:图片标题:图片说明}}}}
   - 在需要插入表格的地方使用：{{{{TABLE:表格标题:表格说明}}}}
   - 图表占位符应放在相关描述文字之后
   - 例如：描述完系统架构后，添加 {{{{FIGURE:系统架构图:展示系统整体模块结构}}}}
"""
...
ABSTRACT_EN_GENERATE_PROMPT = """你是一位学术论文翻译专家。你的任务是将中文摘要准确翻译成英文摘要。
...
请将上述中文摘要翻译成英文。这是一项**翻译任务**，不是改写任务。

## 翻译要求

1. **忠实原文**：准确翻译中文摘要的每一句话，不遗漏、不增加内容
2. **学术英语**：使用正式的学术英语表达
3. **专业术语**：技术术语使用标准的英文表达
4. **自然流畅**：在忠实原文的基础上，确保英文表达地道自然
5. **格式对应**：保持与中文摘要相同的段落结构

## 输出格式

直接输出英文摘要内容，不要加"Abstract"标题。
"""
...
def build_chapter_draft_prompt(
    ...
    for i, child in enumerate(chapter.children, 1):
        ...
        if child.figures:
            for fig in child.figures:
                ...
                subsections_lines.append(f"   - 需插入图片: {{{{FIGURE:{fig.caption}:{fig_desc}}}}}")
        # 添加表格信息
        if child.tables:
            for tab in child.tables:
                tab_desc = tab.description or ""
                subsections_lines.append(f"   - 需插入表格: {{{{TABLE:{tab.caption}:{tab_desc}}}}}")
    ...
    if chapter.figures:
        for fig in chapter.figures:
            fig_desc = fig.description or ""
            subsections_lines.append(f"本章需插入图片: {{{{FIGURE:{fig.caption}:{fig_desc}}}}}")
    if chapter.tables:
        for tab in chapter.tables:
            tab_desc = tab.description or ""
            subsections_lines.append(f"本章需插入表格: {{{{TABLE:{tab.caption}:{tab_desc}}}}}")
